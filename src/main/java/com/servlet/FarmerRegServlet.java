...
public class FarmerRegServlet extends HttpServlet 
{
	...
	protected void service(HttpServletRequest req,HttpServletResponse res) throws IOException, ServletException
	{
	    ...
	    try {
			...
			PreparedStatement ps= con.prepareStatement("insert into farmer_Registration (fullname,dob,contact_number,email,username,password,full_address,city,state,pincode)"+" values(?,?,?,?,?,?,?,?,?,?)");
			...
			if(count > 0)
				...
				RequestDispatcher rd =req.getRequestDispatcher("/Farmer_Reg.jsp");
				rd.include(req, res);
				}
		  }
	    catch(Exception e)
